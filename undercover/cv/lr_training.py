...
dfvalid = dfvalid[['Identifiant_Produit','Categorie3','Categorie1','txt']]
...
def submit(df,Y,mid='.test'):
    submit_file = ddir+'resultat.auto'+mid+ext+'.csv'
    df['Id_Produit']=df['Identifiant_Produit']
    df['Id_Categorie'] = Y
    df= df[['Id_Produit','Id_Categorie']]
    df.to_csv(submit_file,sep=';',index=False)

def save_proba(df,Y,p1,p3,mid='.test'):
    submit_file = ddir+'proba.auto'+mid+ext+'.csv'
    df['Id_Produit']=df['Identifiant_Produit']
    df['Id_Categorie'] = Y
...
proba_cat1_valid =  np.exp(np.max(stage1_log_proba_valid,axis=1))
proba_cat3_valid =  np.exp(np.max(stage3_log_proba_valid,axis=1))

dftest = pd.read_csv(ddir+'test_normed.csv',sep=';',names = header(test=True)).fillna('')
save_proba(dftest,predict_cat3_test,proba_cat1_test,proba_cat3_test,'.test')
save_proba(dfvalid,predict_cat3_valid,proba_cat1_valid,proba_cat3_valid,'.valid')
