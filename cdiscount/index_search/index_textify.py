...
def get_txt_without_color(r):
    txt = (r.Marque.lower()+' '+r.Libelle.lower()+' '+r.Description).split()
...
df = pd.read_csv(ddir+'training_cdiscount_normed.csv',sep=';',names=['Identifiant_Produit','Categorie1','Categorie2','Categorie3','Description','Libelle','Marque','Produit_Cdiscount','prix']).fillna('')
...
sdf = sdf[map(lambda t:len(t)>45,sdf.t_index)]
...
df = df[map(lambda t:len(t)>45,df.t_index)]
